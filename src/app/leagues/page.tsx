...
export default async function LeaguesPage({
  ...
  const { data: leagues } = await supabase
    .from("leagues")
    .select("id,name,description,visibility,scoring_type,perfect_bonus,league_members(count)")
        ...
        <section className="card">
          ...
          <div className="space-y-4">
            {leagues?.length ? (
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <Link href={`/leagues/${league.id}`} className="text-xl font-black text-white hover:text-blue-300">
                          {league.name}
                        </Link>
                        ...
                        <p className="mt-2 text-sm text-blue-300">
                          Scoring: {league.scoring_type} · Perfect bonus: {league.perfect_bonus} · Members: {memberCount}/30
                        </p>
                      </div>
                    ...
                    <Link href={`/leagues/${league.id}`} className="mt-3 inline-block text-sm font-bold text-blue-300 hover:text-blue-100">
                      View members
                    </Link>

                    {!user && (
                      <p className="mt-3 text-sm text-slate-400">
                        Login or create an account to join this league.
                      </p>
                    )}
                  </div>
                );
              ...
              <p className="text-slate-300">No public leagues have been created yet.</p>
            )}
          </div>
        </section>
          ...
          {user ? (
            <form action={createLeague} className="space-y-4">
              ...
              <label>
                ...
                <input name="fixed_points" type="number" defaultValue="1" min="1" />
              </label>

              <label>
                Perfect Event Bonus
                <input name="perfect_bonus" type="number" defaultValue="5" min="0" />
              </label>
            ...
            <p className="text-slate-300">Login to create a league.</p>
          )}
        </section>
      </div>
    </main>
