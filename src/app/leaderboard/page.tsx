...
export default async function LeaderboardPage({
  ...
  if (!user) {
    return (
      <main className="page">
        <PageHero title="Leaderboard" subtitle="Login to view your league standings." />
      </main>
    );
  }
  ...
  if (membershipsError) {
    return (
      <main className="page">
        <PageHero title="Leaderboard" subtitle="League-scoped standings and player stats." />
      ...
      {leagues.length === 0 ? (
        <section className="card">
          ...
          <p className="mt-2 text-slate-300">
            Join or create a league first. Leaderboards are only available for leagues you belong to.
          </p>
        </section>
      ) : (
        <>
              ...
              <tbody>
                {rows && rows.length > 0 ? (
                  rows.map((row: any, index: number) => (
                    <tr key={`${row.league_id}-${row.user_id}`} className="border-b border-slate-900">
                      <td className="p-3 font-black text-blue-200">#{index + 1}</td>
                      <td className="p-3 font-bold">{row.display_name}</td>
                      <td className="p-3 font-black text-red-300">{row.total_points}</td>
                  ...
                  <tr>
                    <td className="p-4 text-slate-300" colSpan={7}>
                      No finalized results yet. An LM or ALM must set match winners and save the event as final before points populate here.
                    </td>
                  </tr>
                )}
              </tbody>
